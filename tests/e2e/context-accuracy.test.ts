...
describe('Context Accuracy E2E Test', () => {
  ...
  function validateProjectContext(
    ...
    if (expectations.boardCount !== undefined) {
      ...
      if (context.boards.length === expectations.boardCount) {
        ...
        result.passed = false;
        result.details.push(
          `✗ Board count mismatch: expected ${expectations.boardCount}, got ${context.boards.length}`
        );
      }
    }
    ...
    if (expectations.taskCount !== undefined) {
      ...
      if (context.key_metrics.total_tasks === expectations.taskCount) {
        ...
        result.passed = false;
        result.details.push(
          `✗ Task count mismatch: expected ${expectations.taskCount}, got ${context.key_metrics.total_tasks}`
        );
      }
    }
    ...
    if (expectations.completedTasks !== undefined) {
      ...
      if (context.key_metrics.completed_tasks === expectations.completedTasks) {
        ...
        result.passed = false;
        result.details.push(
          `✗ Completed tasks mismatch: expected ${expectations.completedTasks}, got ${context.key_metrics.completed_tasks}`
        );
      }
    }
    ...
    if (expectations.blockedTasks !== undefined) {
      ...
      if (context.blockers.length === expectations.blockedTasks) {
        ...
        result.passed = false;
        result.details.push(
          `✗ Blocked tasks mismatch: expected ${expectations.blockedTasks}, got ${context.blockers.length}`
        );
      }
    }
    ...
    if (expectations.overdueTasks !== undefined) {
      ...
      if (context.overdue_tasks.length === expectations.overdueTasks) {
        ...
        result.passed = false;
        result.details.push(
          `✗ Overdue tasks mismatch: expected ${expectations.overdueTasks}, got ${context.overdue_tasks.length}`
        );
      }
    }
    ...
    if (expectations.priorityTasks !== undefined) {
      totalChecks++;
      const highPriorityCount = context.priorities.filter(
      ...
      if (highPriorityCount === expectations.priorityTasks) {
        ...
        result.passed = false;
        result.details.push(
          `✗ High priority tasks mismatch: expected ${expectations.priorityTasks}, got ${highPriorityCount}`
        );
      }
    }
  ...
  test('should accurately detect blocked tasks and dependencies', async () => {
    ...
    const highPriorityTasks = context.priorities.filter(
      p => p.urgency_level === 'high' || p.urgency_level === 'critical'
    );
