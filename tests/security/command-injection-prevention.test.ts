...
describe('Command Injection Prevention Verification', () => {
  ...
  describe('Command Validation', () => {
    ...
    test('should enforce total arguments length limits', () => {
      ...
      expect(result.safe).toBe(false);
      expect(result.blockedPatterns.some(p => p.includes('Total arguments length exceeds maximum'))).toBe(true);
    });
  ...
  describe('Safe Command Execution', () => {
    ...
    test('should block dangerous command execution', async () => {
      await expect(cip.safeExecute('rm', ['-rf', '/'])).rejects.toThrow('Command execution blocked');
    });

    test('should block command injection in arguments', async () => {
      for (const payload of COMMAND_INJECTION_PAYLOADS.slice(0, 5)) {
        await expect(cip.safeExecute('echo', [payload])).rejects.toThrow('Command execution blocked');
      }
    });
  ...
  describe('File Path Validation', () => {
    ...
    test('should enforce allowed directory restrictions', () => {
      ...
      const safeResult = CommandInjectionPrevention.validateFilePath('/home/user/safe/file.txt', allowedDirs);
  ...
  describe('Advanced Attack Scenarios', () => {
    ...
    test('should prevent environment variable exploitation', () => {
      ...
      envPayloads.forEach(payload => {
        ...
        expect(result.safe).toBe(false);
        expect(result.blockedPatterns.some(p => 
          p.includes('Dangerous pattern') || p.includes('Path traversal')
        )).toBe(true);
      });
    });
  });
  ...
  describe('Real-world CLI Integration', () => {
    test('should safely handle git commands', async () => {
      ...
      await expect(
        cip.safeExecute('git', ['log', '; rm -rf /'])
      ).rejects.toThrow('Command execution blocked');
    });
    ...
    test('should maintain security with working directory restrictions', () => {
      ...
      expect(absoluteResult.blockedPatterns.some(p => 
        p.includes('outside working directory')
      )).toBe(true);
    });
  });
