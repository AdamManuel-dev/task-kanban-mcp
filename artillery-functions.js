...
module.exports = {
  ...
  logMetrics: function(context, events, done) {
    events.on('request', function(requestParams, response, context, ee, next) {
      ...
      if (responseTime > 1000) {
        console.log(`SLOW REQUEST: ${requestParams.method} ${requestParams.url} - ${responseTime}ms`);
      }
  ...
  generateTaskData: function(context, events, done) {
    ...
    const titles = {
      bug: ['Fix login authentication bug', 'Resolve database connection issue', 'Fix UI rendering problem'],
      feature: ['Add user profile page', 'Implement search functionality', 'Create dashboard widgets'],
