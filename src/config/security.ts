...
export const securityConfigSchema = z.object({
  ...
  cors: z.object({
    origin: z
      .union([z.string(), z.array(z.string()), z.function().returns(z.boolean())])
      .refine(val => {
    ...
    credentials: z.boolean().refine(val => {
      if (process.env.NODE_ENV === 'production' && val) {
...
export function hashApiKey(apiKey: string, secret: string): string {
  return crypto.createHmac('sha512', secret).update(apiKey).digest('hex');
}
