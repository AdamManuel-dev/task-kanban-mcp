...
/**
 * Initial schema migration - Creates all base tables, indexes, views, and triggers
 * This migration sets up the complete MCP Kanban database structure as defined in the PRD
 */
...
export async function up(db: Database): Promise<void> {
  ...
  await run(`CREATE INDEX IF NOT EXISTS idx_tasks_board_id ON tasks(board_id)`);
  await run(`CREATE INDEX IF NOT EXISTS idx_tasks_column_id ON tasks(column_id)`);
  await run(`CREATE INDEX IF NOT EXISTS idx_tasks_status ON tasks(status)`);
  await run(`CREATE INDEX IF NOT EXISTS idx_tasks_priority ON tasks(priority DESC)`);
  ...
  await run(`CREATE INDEX IF NOT EXISTS idx_task_tags_task_id ON task_tags(task_id)`);
  ...
  await run(`CREATE INDEX IF NOT EXISTS idx_columns_position ON columns(position)`);
  await run(`CREATE INDEX IF NOT EXISTS idx_tags_parent_tag_id ON tags(parent_tag_id)`);
...
export async function down(db: Database): Promise<void> {
  ...
  await run(`DROP TRIGGER IF EXISTS tasks_updated_at`);
  await run(`DROP TRIGGER IF EXISTS notes_updated_at`);
  await run(`DROP TRIGGER IF EXISTS repository_mappings_updated_at`);
  await run(`DROP TRIGGER IF EXISTS tasks_fts_insert`);
  await run(`DROP TRIGGER IF EXISTS tasks_fts_update`);
  await run(`DROP TRIGGER IF EXISTS tasks_fts_delete`);
  await run(`DROP TRIGGER IF EXISTS notes_fts_insert`);
  await run(`DROP TRIGGER IF EXISTS notes_fts_update`);
  await run(`DROP TRIGGER IF EXISTS notes_fts_delete`);

  // Drop views
  await run(`DROP VIEW IF EXISTS board_stats`);
  await run(`DROP VIEW IF EXISTS task_dependency_graph`);
  await run(`DROP VIEW IF EXISTS active_tasks`);

  // Drop FTS tables
  await run(`DROP TABLE IF EXISTS notes_fts`);
  await run(`DROP TABLE IF EXISTS tasks_fts`);

  // Drop tables in reverse dependency order
  await run(`DROP TABLE IF EXISTS schema_info`);
  await run(`DROP TABLE IF EXISTS api_keys`);
  await run(`DROP TABLE IF EXISTS task_history`);
  await run(`DROP TABLE IF EXISTS task_time_entries`);
  await run(`DROP TABLE IF EXISTS repository_mappings`);
  await run(`DROP TABLE IF EXISTS task_tags`);
  await run(`DROP TABLE IF EXISTS tags`);
  await run(`DROP TABLE IF EXISTS notes`);
  await run(`DROP TABLE IF EXISTS task_dependencies`);
