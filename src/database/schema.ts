...
export class SchemaManager {
  ...
  async validateSchema(): Promise<SchemaValidationResult> {
    ...
    try {
      ...
      if (currentVersion !== SchemaManager.SCHEMA_VERSION) {
        result.errors.push(`Schema version mismatch: expected ${SchemaManager.SCHEMA_VERSION}, got ${currentVersion}`);
        result.isValid = false;
      }
  ...
  async dropSchema(): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Schema drop failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private extractObjectNames(sql: string, createStatement: string): string[] {
    const regex = new RegExp(`${createStatement}\\s+(?:IF NOT EXISTS\\s+)?(?:\\w+\\.)?([\\w_]+)`, 'gi');
