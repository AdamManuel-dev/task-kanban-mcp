...
router.post('/import', requirePermission('write'), async (_req: Request, res: Response, next: NextFunction) => {
  try {
    return res.status(501).apiError('Import functionality requires multer middleware', 'NOT_IMPLEMENTED');
  } catch (error) {
    return next(error);
  }
});
...
router.post('/import/validate', requirePermission('read'), async (_req: Request, res: Response, next: NextFunction) => {
  try {
    return res.status(501).apiError('Import validation requires multer middleware', 'NOT_IMPLEMENTED');
  } catch (error) {
    return next(error);
  }
});
