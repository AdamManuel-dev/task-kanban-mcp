...
export class ResponsiveDesignHelper {
   ...
   */
  private static calculateBoardLayout(size: TerminalSize, breakpoint: BreakpointConfig): ResponsiveLayout {
   ...
   */
  private static calculateListLayout(size: TerminalSize, breakpoint: BreakpointConfig): ResponsiveLayout {
   ...
   */
  private static calculateTableLayout(size: TerminalSize, breakpoint: BreakpointConfig): ResponsiveLayout {
  ...
  static adaptForMobile<T extends { title: string; description?: string }>(
    ...
    if (truncateTitles) {
      ...
      adapted = adapted.map(item => ({
        ...item,
        title: item.title.length > maxTitleLength 
   ...
   */
  static createGridLayout(items: any[], columns: number): any[][] {
    const grid: any[][] = [];
