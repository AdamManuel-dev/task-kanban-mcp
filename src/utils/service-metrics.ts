...
class ServiceMetricsCollector {
  ...
  generateSummaryReport(): {
    ...
    return {
      ...
      totalCalls,
      averageHealthScore: services.reduce((sum, s) => sum + s.healthScore, 0) / services.length,
...
export function TrackPerformance(serviceName?: string) {
  return function <T>(
    ...
    if (process.env.NODE_ENV === 'test' || process.env.JEST_WORKER_ID) {
      return descriptor ?? { configurable: true, writable: true }; // eslint-disable-line @typescript-eslint/no-unnecessary-condition
    }
